...
function kannelParameter($is_flash, $is_unicode)
{
    if($is_flash==1)
    {
        ...
        $dlr_mask = 31;
        $charset = ($is_unicode==1) ? 'UTF-8' : 'ISO-8859-1';
        //$dlr_mask = 3;
    }
    else
    {
...
function checkApiCampaignComplete()
{
    ...
    $statsSubQuery = DB::table('send_sms_queues')
        ->select(
            'send_sms_id',

            DB::raw("SUM(stat = 'DELIVRD') as total_delivered"),
            DB::raw("SUM(stat = 'Invalid') as total_invalid"),
            DB::raw("SUM(stat = 'BLACK') as total_block"),
            DB::raw("SUM(stat NOT IN ('DELIVRD','Invalid','BLACK','Pending','Accepted')) as total_failed")
        )
        ->groupBy('send_sms_id');


    DB::table('send_sms as sms')
    ->joinSub($statsSubQuery, 'q', function ($join) {
        $join->on('sms.id', '=', 'q.send_sms_id');
    })
    ->where('sms.status', 'Ready-to-complete')
    ->whereDate('sms.campaign_send_date_time', $yesterday)
    ->where('sms.campaign', 'API')
    ->update([
        'sms.total_delivered'      => DB::raw('q.total_delivered'),
        'sms.total_invalid_number' => DB::raw('q.total_invalid'),
        'sms.total_block_number'   => DB::raw('q.total_block'),
        'sms.total_failed'         => DB::raw('q.total_failed'),

        'sms.status' => DB::raw("
            CASE
                WHEN sms.total_contacts <= 
                    (q.total_delivered + q.total_invalid + q.total_block + q.total_failed)
                THEN 'Completed'
                ELSE sms.status
            END
        "),
