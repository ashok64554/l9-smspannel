...
class ChatbotEngine
{
     ...
     *
     * matching_criteria: exact | contain | regex | default
     */
    protected function detectInitiationBot($bots, string $message, bool $hasActiveSession = false)
    {
        ...
        foreach ($sortedBots as $bot) {
            ...
            $startWith = strtolower(trim($bot->start_with ?? ''));

            /**
             * ==============================
             * DEFAULT BOT (ONLY IF NO SESSION)
             * ==============================
             */
            if (!$hasActiveSession && ($criteria === 'default' || $bot->is_default)) {
                if (!$defaultBot) {
                    $defaultBot = $bot;
                }
                continue;
             ...
             * ==============================
             * Only EXACT match can trigger switch
             */
            ...
            /**
             * ==============================
             * CONTAIN MATCH (NO SESSION)
             * ==============================
             */
            if (!$hasActiveSession && $criteria === 'contain') {
            ...
            /**
             * ==============================
             * REGEX MATCH (NO SESSION)
             * ==============================
             */
            if (!$hasActiveSession && $criteria === 'regex') {
                try {
