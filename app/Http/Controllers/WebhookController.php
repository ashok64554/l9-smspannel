...
class WebhookController extends Controller
{
        ...
        try {
            ...
            if($default=='Direct')
            {
                ...
                elseif(array_key_exists('messages', $getStatus))
                {
                    ...
                    $profileName = @$getStatus['contacts'][0]['profile']['name'];
                    ...
                    $getConfId = DB::table('whats_app_configurations')
                        ->where('display_phone_number', @$getStatus['metadata']['display_phone_number'])
                        ->orWhere('display_phone_number_req', @$getStatus['metadata']['display_phone_number'])
                        ->first();
                    /*
                    ...
                    */
                    ...
                    if($getConfId)
                    {
                        ...
                        if(!empty($text))
                        {
                            ...
                            $response = $engine->handleIncomingMessage($from, $profileName, $text, $configId, $getStatus);
                        }
                    }
    ...
    // This is for showing records
    public function waWebhookSessionApiRecord(Request $request)
    {
        /*
        $sessionResponse = WhatsAppChatBotSessionApiRecord::create([
            'whats_app_configuration_id' => $bot->id,
