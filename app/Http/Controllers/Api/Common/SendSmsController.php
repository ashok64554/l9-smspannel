...
class SendSmsController extends Controller
{
                    ...
                    foreach ($chunks->toArray() as $chunk) 
                    {
                        ...
                        foreach ($chunk as $number) 
                        {
                            ...
                            if(@$checkNumberValid['is_auto']==0 && @$checkNumberValid['number_status']==1)
                            {
                                ...
                                $dlr_url = 'http://'.$kannel_ip.':'.$node_port.'/DLR?msgid='.$unique_key.'&d=%d&oo=%O&ff=%F&s=%s&ss=%S&aa=%A&wh_url='.$wh_url.'&send_sms_id='.$sendSMS->id;
                                $kannelData[] = [
                                    'momt' => 'MT',
                                    'sender' => $sender_id,
    ...
    public function repushCampaign(Request $request)
    {
        ...
        try {
            ...
            $sendSmsId = $repushSendSMS->id;
            $queue = SendSmsQueue::where('send_sms_id', $repushSendSMS->id)
            ->chunk(env('CHUNK_SIZE', 1000), function ($records) use ($kannelPara, $meta_data, $kannel_ip, $node_port, $smsc_id, $priority, $sender_id, $ratio_percent_set, $failed_ratio,$wh_url,$sendSmsId)
            {
                $kannelData = [];
                ...
                foreach ($records as $rec) 
                {
                    ...
                    if($isNotInRatio==true)
                    {
                        ...
                        $dlr_url = 'http://'.$kannel_ip.':'.$node_port.'/DLR?msgid='.$unique_key.'&d=%d&oo=%O&ff=%F&s=%s&ss=%S&aa=%A&wh_url='.$wh_url.'&send_sms_id='.$sendSmsId;
                        $kannelData[] = [
                            'momt' => 'MT',
                            'sender' => $sender_id,
                            'receiver' => $rec->mobile,
    ...
    public function resendThisSms(Request $request)
    {
        ...
        try {
            ...
            $dlr_url = 'http://'.$kannel_ip.':'.$node_port.'/DLR?msgid='.$findSMS->unique_key.'&d=%d&oo=%O&ff=%F&s=%s&ss=%S&aa=%A&wh_url='.$wh_url.'&send_sms_id='.$smsInfo->id;
            $kannelData[] = [
                'momt' => 'MT',
