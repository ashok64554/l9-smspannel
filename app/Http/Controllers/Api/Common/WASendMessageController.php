...
class WASendMessageController extends Controller
{
    ...
    public function index(Request $request)
    {
        try {
            ...
            ->leftjoin('whats_app_templates', 'whats_app_send_sms.whats_app_template_id', 'whats_app_templates.id')
                                    ...
                                    if($waVariable=='header')
                                    {
                                        if($template_type=='MEDIA')
                                        {
                                            ...
                                            if(!empty($number['media_var_1']))
                                            {
                                                ...
                                                $titleOrFileName = $number['caption_var_1'];
                                                $header = prapareWAComponent($waVariable, $number['media_var_1'], $parameter_format, null, $template_type, $media_type, $titleOrFileName,$latitude,$longitude,$location_name,$location_address,$wa_file_id);
                                            }
                                            else
                                            {
                                                $titleOrFileName = $request->file_caption;
            ...
            if($getRecord)
            {
                ...
                if($response['error']==false)
                {
                    ...
                    $wa_reply_threads[] = [
                        ...
                        'phone_number_id' => $getRecord->display_phone_number_req,
