...
class ReportController extends Controller
{
    ...
    public function detailedReport(Request $request)
    {
        ...
        try
        {
            $dlrSub = DB::table('dlrcode_venders')
                ->select('dlr_code', DB::raw('MIN(description) as description'))
                ->groupBy('dlr_code');
            $from = $request->from . ' 00:00:00';
            $to   = $request->to   . ' 23:59:59';

            $buildAgg = function ($table) use ($from, $to, $request) 
            {
                $q = DB::table("$table as ssq")
                    ->select('ssq.err', DB::raw('COUNT(*) as total_count'))
                ...
                if (in_array(loggedInUserType(), [0,3])) {
                    if (!empty($request->user_id)) {
                        $q->whereExists(function ($s) use ($request) {
                            $s->select(DB::raw(1))
                              ->from('send_sms')
                              ->whereColumn('send_sms.id', 'ssq.send_sms_id')
                    ...
                    $q->whereExists(function ($s) use ($uid) {
                        $s->select(DB::raw(1))
                          ->from('send_sms')
                          ->whereColumn('send_sms.id', 'ssq.send_sms_id')
            ...
            if ($toDate->gte($todayStart)) {
                $queries[] = $buildAgg('send_sms_queues');
            }

            if ($fromDate->lt($todayStart)) {
                $queries[] = $buildAgg('send_sms_histories');
            }
            ...
            $data = DB::query()
                ->fromSub($base, 'x')
                ->leftJoinSub($dlrSub, 'dv', 'x.err', '=', 'dv.dlr_code')
                ->select(
                    'x.err',
                    'dv.description',
                    DB::raw('SUM(x.total_count) as total_count')
                )
                ->groupBy('x.err', 'dv.description')
