...
class WAReportController extends Controller
{
    ...
    //// All responses
    public function waChatbotsSelfWebhook(Request $request)
    {
        try {
            $query = WhatsAppChatBotSessionApiRecord::select('whats_app_chat_bot_session_api_records.*', 'whats_app_configurations.display_phone_number', 'whats_app_configurations.name')
            ->orderBy('whats_app_chat_bot_session_api_records.id', 'DESC')
            ->leftjoin('whats_app_configurations', 'whats_app_chat_bot_session_api_records.whats_app_configuration_id', 'whats_app_configurations.id')
            ->orderBy('whats_app_chat_bot_session_api_records.id', 'DESC');
            ...
            {
                $query->where('whats_app_chat_bot_session_api_records.whats_app_configuration_id', $request->whats_app_configuration_id);
            }
