...
class WAConfigurationController extends Controller
{
    ...
    public function __construct()
    {
        ...
        $this->middleware('permission:wa-configuration-assign-to-child', ['only' => ['waConfigurationAssignToChild']]);
    }
    ...
    */
    ...
    {
        $getConfInfo = WhatsAppConfiguration::where('user_id', 2)->orWhere('name', 'Ashok Sahu WA')->find($whats_app_configuration_id);
        if(!$getConfInfo)
        {
            return response()->json(prepareResult(true, [], trans('translate.record_not_found'), $this->intime), config('httpcodes.not_found'));
        }
    ...
    public function waMarketingLiteApiStatus(Request $request, $whats_app_configuration_id)
    {
        try {
                if(in_array(loggedInUserType(), [1,2]))
                {
                ...
                $url = "https://graph.facebook.com/$appVersion/$waba_id/?fields=marketing_messages_lite_api_status";
                ...
                {
                    $response = json_decode($response->body(), true);
                    $getConfInfo->marketing_messages_lite_api_status = $response['marketing_messages_lite_api_status'];
                    $getConfInfo->mm_lite_request_id = $response['id'];
                    $getConfInfo->save();
                    return response()->json(prepareResult(false, $response, trans('translate.fetched_records'), $this->intime), config('httpcodes.success'));
                }
                else
    ...
    public function onboardPartnersToMMLite(Request $request, $whats_app_configuration_id)
    {
        try {
                if(in_array(loggedInUserType(), [1,2]))
                {
                ...
                if($getConfInfo->marketing_messages_lite_api_status!='ELIGIBLE')
                {
                    return response()->json(prepareResult(true, [], trans('translate.not_allowed_user_to_onboard_to_mm_lite_api'), $this->intime), config('httpcodes.not_found'));
                }

                if($getConfInfo->marketing_messages_lite_api_status=='ONBOARDED')
                {
                    return response()->json(prepareResult(true, [], trans('translate.already_onboarded'), $this->intime), config('httpcodes.not_found'));
                }
                ...
                $url = "https://graph.facebook.com/$appVersion/$business_account_id/onboard_partners_to_mm_lite";
                $payload = [
                ...
                {
                    $response = json_decode($response->body(), true);
                    $getConfInfo->mm_lite_request_id = $response['request_id'];
                    $getConfInfo->is_mm_lite_request_sent = 1;
                    $getConfInfo->save();
                    return response()->json(prepareResult(false, $response, trans('translate.fetched_records'), $this->intime), config('httpcodes.success'));
                }
                else
    ...
    public function waConfigurationAssignToChild(Request $request)
    {
        $validation = \Validator::make($request->all(), [
        ...
        try {
                ...
                {
                    $getConfInfo = WhatsAppConfiguration::find($request->whats_app_configuration_id);
                }
                ...
                {
                    $user_id = auth()->id();
                    $checkUser = User::whereIn('id', $request->user_ids)->where('parent_id', $user_id)->get();
                }
                else
                {
                    $checkUser = User::whereIn('id', $request->user_ids)->get();
                }
                ...
                if(sizeof($checkUser) != sizeof($request->user_ids))
                {
                    return response()->json(prepareResult(true, [], trans('translate.some_record_not_found'), $this->intime), config('httpcodes.not_found'));
                }
                foreach ($request->user_ids as $key => $user_id) 
                {
                    //check duplicate
                    if(WhatsAppConfiguration::where('user_id', $user_id)->where('sender_number', $getConfInfo->sender_number)->count() < 1)
                    {
                        $getConfInfo = $getConfInfo->replicate();
                        $getConfInfo->user_id = $user_id;
                        $getConfInfo->save();
                    }
                }
                
                return response()->json(prepareResult(false, [], trans('translate.assigned'), $this->intime), config('httpcodes.success'));
