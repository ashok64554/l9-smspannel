...
class ApiGetController extends Controller
{
                ...
                ******************************************/
                ...
                foreach ($chunks as $chunk) 
                {
                    ...
                    foreach ($chunk as $number) 
                    {
                        ...
                        {
                            $dlr_url = 'http://'.$kannel_ip.':'.$node_port.'/DLR?msgid='.$unique_key.'&d=%d&oo=%O&ff=%F&s=%s&ss=%S&aa=%A&wh_url='.$wh_url.'&uuid='.$sendSMS->uuid.'&mobile='.$checkNumberValid['mobile_number'].'&used_credit='.$messageSizeInfo['message_credit_size'].'&send_sms_id='.$sendSMS->id;
                            $kannelData[] = [
                                'momt' => 'MT',
                                'sender' => $sender_id,
    ...
    public function sendWaMessage(Request $request)
    {
        try {
            ...
            $marketing_messages_lite_api_status  = $whatsAppConfiguration->marketing_messages_lite_api_status;
