...
class SmsDetailedCsvZipJob implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        $dlrSub = DB::table('dlrcode_venders')
            ->select('dlr_code', 'description')
            ->groupBy('dlr_code');

        $buildQuery = function ($table, $dlrSub) {
            ...
            $q = DB::table("$table as ssq")
                ...
                ->leftJoinSub($dlrSub, 'dv', function ($join) {
                    $join->on('ssq.err', '=', 'dv.dlr_code');
                })
                ->select(
                    'u.name as user_name',
        ...
        if ($onlyToday) {
            $queries[] = $buildQuery('send_sms_queues', $dlrSub);
        } elseif ($onlyPast) {
            $queries[] = $buildQuery('send_sms_histories', $dlrSub);
        } else {
            $queries[] = $buildQuery('send_sms_queues', $dlrSub);
            $queries[] = $buildQuery('send_sms_histories', $dlrSub);
        }
